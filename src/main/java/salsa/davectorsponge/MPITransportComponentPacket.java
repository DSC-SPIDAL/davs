...
/**
 * The following values are stored as offsets in the <code>ByteBuffer</code> object.
 * The structure of the buffer is
 *
 *--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
 * #clus | #doubleComps | #intComps | clusIntCompsOffset | assocCreatedIdxOffset | clusHostRangeOffset | clusDoubleComps[] | clusIntComps[] | assocCreatedIdx[] | clusHostRange[]
 *--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
 *
 * numberOfClusters -- Number of Clusters in Package
 * numberOfDoubleComponents -- Number of Double Components in Package
 * numberOfIntegerComponents; // Number of Integer Components in Package
 * clusterDoubleComponents -- double array -- Transported Double Components
 * clusterIntegerComponents -- int array --  Transported Integer Components
 * associatedCreatedIndex -- int array -- Created Index of Cluster
 * clusterHostRange -- int array -- Hosting node Intended for Cluster in form H + MULTIPLIER (H1 + MULTIPLIER * H2 )
 *                                  Note H1 <= H <= H2
 *                                  Near End either H1 = H2 = H or H1 = H - 1 and/or H2 = H + 1
 *
 * Note. Offset to clusterDoubleComponents array is not stored in the buffer to save space as it
 *       is always 6 * Integer.BYTES
 */
